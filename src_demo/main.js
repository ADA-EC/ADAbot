...
//Grupo do bot
//const group = '5516992027508-1623887349@g.us'
//Grupo da ADA
const group = '5511986872544-1585855722@g.us';
...
venom
  ...
  async function get_events(){
    ...
    try {
      ...
      client
.sendText(group, "Proximo: " + info[0]["summary"])
.then((result) => {
...
async function start(client) {
  ...
  setInterval(async function foo() {
      const Messages = await client.getAllMessagesInChat(group);
      ...
      if (lastMessage.search("@5516992027508") > -1) {
          ...
          if (lastMessage.search("evento") > -1) {
              ...
              client
                  .sendText(group, "Oi, eu sou o ADABOT. Abaixo você encontrará uma lista de coisas que você pode me pedir:\n" + "=================================")
                  .then((result) => {
                      //console.log('Result: ', result); //return object success
                  })
                  .catch((erro) => {
              ...
              client
                  .sendText(group, "Proximo numero do Bingo: " + num)
                  .then((result) => {
                      //console.log('Result: ', result); //return object success
                  })
                  .catch((erro) => {
              ...
              client
                  .sendText(group, "Desculpe, eu não entendi. Você pode me marcar e escrever 'help' para ver a lista de comandos implementados")
                  .then((result) => {
                      //console.log('Result: ', result); //return object success
                  })
                  .catch((erro) => {
  ...
  let messageTimer = new Timer(() => {
      client.sendText(group, 'Voces sabem que sao meus unicos amigos? 😭');
